...
class _TimeLineLeftState extends State<TimeLineLeft> {
  ...
  Map schedule={
    'Oct1': {
      'Time':'8:53PM to 10:53PM',
      'Details':'Nomination caucus',
    }
  };
  ...
  Widget circle(var count){
    return Padding(
      padding: EdgeInsets.only(bottom: widget.height/6 - widget.width/9 ,top: 0),
      child: CircleAvatar(
      ...
      return  Container(
           height: widget.height/6,
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceAround,
              children:<Widget>[
                (!widget.swapTimeLine)?Container(
                  width: widget.width/8+2,
                  height: widget.height/6,
                  child: Stack(children:<Widget>[line(index+1),circle(index+1)])) : Expanded(child: event())
                ,
                (!widget.swapTimeLine)?Expanded(child: event()):Container(
                    width: widget.width/8+2,
                    height: widget.height/6,
                   // padding: const EdgeInsets.only(left: 20),
                    child: Stack(children:<Widget>[line(index+1),circle(index+1)])),
        ]
        ));
    });
